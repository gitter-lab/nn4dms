...
def run_inference(encoded_data, ckpt_prefix_fn=None, sess=None, output_tensor_name="output/BiasAdd:0", batch_size=64):
    ...
    predictions = tf.squeeze(sess.graph.get_tensor_by_name(output_tensor_name))
    ...
    if encoded_data.shape[0] > batch_size:
        ...
        for batch_num, batch_data in tqdm(enumerate(bg), total=num_batches):
            ...
            predictions_for_data[start_index:end_index] = predictions_for_batch
    else:
        predictions_for_data = sess.run(predictions, feed_dict={input_seqs_ph: encoded_data})
